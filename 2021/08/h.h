...
#define SENTENCES 10
//#define SENTENCES 199
#define SENTENCE_LENGTH 58
#define SECRET_WORD_COUNT 4

typedef struct {
    // Each letter represents an encoded segment. Initially we assume 
    // this segment can be any number. With each word in our code, we
    // can filter out impossible segments. Hopefully, after 10 words,
    // there is only one single "true" per each encoded letter.
    // segment = vtl,  vtr,  vbl,  vbr,  ht,   hm,   hb
    bool segments[][7] = {
        ...
        {true, true, true, true, true, true, true}  // g
    };
} SevenSegmentDisplay;

SevenSegmentDisplay* newSevenSegmentDisplay();
void toggleSegmentToUniqueNumber(SevenSegmentDisplay ssd, int number);
